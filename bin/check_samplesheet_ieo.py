...
def check_samplesheet(file_in, file_out):
    ...
    rid,sid,sample,replicate,path,lanes,experiment
    ...
    with open(file_in, "r", encoding='utf-8-sig') as fin:
        ## Check header
        MIN_COLS = 7
        HEADER = ["rid","sid","sample","replicate","path","lanes","experiment"]
        ...
        for line in fin:
            if line.strip():
                ...
                rid,sid,sample,replicate,path,lanes,experiment = lspl[: len(HEADER)]
    ...
    if len(sample_mapping_dict) > 0:
        ...
        with open(file_out, "w") as fout:
            fout.write(
                ",".join(["sample","single_end", "fastq_1", "fastq_2", "experiment"])
                + "\n"
            )
