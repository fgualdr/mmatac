...
if(opt$norm == TRUE){
    ...
    design = as.data.frame(do.call(rbind,strsplit(colnames(count.table),"_")))
    colnames(design) = paste0("Condition_",1:ncol(design))
    rep = apply(design,2,function(x){
        x = grep("rep[0-9]|R[0-9]",x)
        return(length(x))
    })
    w = which.max(rep)
    cond = apply(design,2,function(x){return(length(unique(x)))})
    w.cond = which(cond == 1)
    w.cond = w.cond[which(w.cond!=w)]
    colnames(design)[w] = "Sample_Replicate"
    design$Sample_ID = colnames(count.table)
    rownames(design) = colnames(count.table)
    design = design[,-w.cond]
    ...
    norm_mat <- count.table[, rownames(colData)]
    norm_mat <- sweep(norm_mat[, colData$samples], 2, colData[, "scaling"], "*")
