...
def read_input_file(path_bedpe):
    ...
    # Import using numpy
    arr = np.loadtxt(path_bedpe, dtype=str)
    # Check that there are 3 columns in the input file and if not raise an error
    if arr.shape[1] != 3:
        raise ValueError('Input file must have 3 columns: read_full_name, read_id, target_id')
    # Rename the column header of the numpy array
    arr = arr.rename(columns={0: 'read_full_name', 1: 'read_id', 2: 'target_id'})
    # get the unique read_ids and their counts
    unique_read_ids, counts = np.unique(arr['read_id'], return_counts=True)
    # create a dictionary mapping read_ids to their counts
    read_id_counts = dict(zip(unique_read_ids, counts))
    # create a new column in the numpy array with the counts of each read_id named counts_per_read_id
    arr['counts_per_read_id'] = arr['read_id'].map(read_id_counts)
    # Get single mappers
    single_mappers = arr[arr['counts_per_read_id'] == 1]
    # Get multi mappers
    multi_mappers = arr[arr['counts_per_read_id'] > 1]
    # Filter multi mappers to keep only those with target_id covered by single mappers
    multi_mappers = multi_mappers[multi_mappers['target_id'].isin(single_mappers['target_id'])]
    # Filter single mappers to keep only those with target_id covered by multi mappers
    single_mappers = single_mappers[single_mappers['target_id'].isin(multi_mappers['target_id'])]
    # Join single and multi mappers
    df = pd.concat([single_mappers, multi_mappers])
...
# The input is a numpy array with 'read_id', 'target_id' and 'read_full_name' as columns.
# These have being already filtered to keep only the rows with target_id in target_ids_single_mappers.
# We exclude thoseraews with target_id == ' id_nomatch' and we count the number of single mappers per target.
def compute_initial_prior(df):
    # Get the single mappers - df is a numpy array with 'read_id', 'target_id' and 'read_full_name' and 'counts_per_read_id' as columns.
