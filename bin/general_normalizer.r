...
opt = list()
opt$count_file    = "consensus_peaks.mLb.clN.featureCounts.txt"
opt$outdir        = "./"
opt$outprefix     = "consensus_peaks.mLb.clN"
opt$sample_suffix = '.Lb.sorted.bam'
opt$id_col        = 1
opt$count_col     = 7
opt$sigma_times           = 1
opt$norm          = TRUE
...
if(opt$norm == TRUE){
    ...
    x = x[round(nrow(x)*0.01):round(nrow(x)*0.99),]
