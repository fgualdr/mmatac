...
workflow BAM_FILTER_BAMTOOLS {
    ...
    // Should modify to account for single end data!! 
    
    // A) Get the fragment length - make the channel 
    // BAM_FRAGMENT emits only the fragment length
    // BAM_FRAGMENT(   ch_bam_bai, 
    //                 ch_bed )
    // ch_versions = ch_versions.mix(BAM_FRAGMENT.out.versions.first())
    // combine the fragment length with the bam file
    // ch_bam_bai
    //     .join(BAM_FRAGMENT.out.fragment_90, by: [0])
    //     .set { ch_bam_frag }

    // FILTER according to the fragment size - this output BEDPE files for single and multi-mappers
    // We use the user specified inser_size from params.inser_size
    ...
    BAM_SORT_STATS_SAMTOOLS(ch_filter_bam,ch_fasta)
